...
\title{\changed{3DIC Power, Signal, and Thermal Pathfinding Using a Virtual Integration Platform}}
...
in high performance 3DICs. In order to understand the 3D design space, and to decide when a 3D system
might have advantages over a 2D system, all of these factors must be modeled simultaneously.
\changed{Performing these coupled simulations with high fidelity is computationally intensive, rendering
...
to enable rapid exploration of the 3DIC design space. \changed{The aim of this work is the creation of a compact simulation tool enabling
...
then be used to guide more precise investigations into the detailed physical design of 3DICs in the appropriate regimes.}

The virtual platform consists of several simulation
...
\changed{In \cref{s-validation} the predictions of the virtual platform are validated against published
...
\changed{The virtual platform source code is available online at http://www.ece.gatech.edu/research/labs/i3ds/}.
...
\changed{The user first inputs some high-level information describing the desired design, including the desired clock frequency,
estimated logic activity factor (the fraction of transistors switching on any given clock cycle), transistor size and expected
leakage parameters, wiring material resistivity (and size effect parameters), expected die area and thickness.
...
In addition to these parameters, the user must also specify the Rent constant, $k$, and Rent exponent, $p$,
which are described in greater detail in \cref{s-interconnect-modeling}.}

\begin{figure}[tb]
	\centering
	\includegraphics[width=3.5in]{Figures/vp-flowchart-3.png}
	\caption{Block diagram of the virtual platform execution flow.}
	\label{f-vp-flowchart}
\end{figure}

\changed{The Rent parameters can be used to determine the number of TSVs required for routing signals within a 3DIC. 
The TSV size and number are then used by the interconnect module to estimate the distribution of 
wire lengths in the system, which is in turn used to determine the number of wiring tiers required for signal routing, 
the pitch of the wires on each tier, and the size and number of repeaters which must be inserted into long wires to meet
the desired clock frequency.}

\changed{
For the purposes of interconnect modeling, the logic block under consideration is treated as a homogeneous design.
This assumption allows the rapid calculation of the on-chip interconnect network parameters with a minimum of a priori
knowledge about the design specifics, at the cost of the loss of insight into the fine-grained details of the design.
In order to simulate complex heterogeneous designs, the user can input the expected parameters and position of each block
in the system, and the virtual platform will model the interconnect networks of each block separately, estimate
the power consumption of each block separately, and then assemble an overall power density map of each tier in the design
for determination of the thermal profile in the entire stack. When modeling heterogeneous designs, the interconnections
between each block are ignored, as it is not possible to estimate the inter-block interconnect requirements without
additional knowledge of the specific design under consideration.
...
\changed{Once the on-chip interconnect parameters are known, the overall capacitance of the wiring network can be determined,
which can be used to estimate the on-chip interconnect power consumption. The dynamic and leakage power consumed by transistors
is also calculated, to determine the overall power requirements for the design. Once the total power consumption is known,
it is used in conjunction with the TSV resistance and inductance (determined from the TSV geometry and material parameters)
and the package pin resistance and inductance (input by the user)
to estimate the simultaneous switching noise in the power delivery network. Additional TSVs (for 3D designs)
or power/ground pads (for 2D designs) are inserted until the simultaneous switching noise drops below a user-determined target.
At this point the virtual platform checks that the total area allocated to TSVs does not exceed a user-specified limit; 
if the demand for TSVs outstrips available area, the virtual platform will reduce the TSV diameter (which may require reducing
the die thickness, if the user specifies that the TSV aspect ratio is fixed) and rerun the interconnect and power modules with
...
boundary condition representing the capabilities of the top-mounted heatsink. If the maximum temperature in the stack
exceeds a user-defined limit, the virtual platform will reduce the clock frequency and rerun the previous modules until
the maximum temperature is sufficiently reduced. The user can also specify that the logic activity factor be reduced,
or that the wire or interlayer dielectric materials be modified, as alternate means of power reduction.
Once the virtual platform determines that the design under consideration satisfies both the TSV area
constraint and the thermal constraint the final design parameters are reported to the user.}
...
in order to determine the overall wiring capacitance, which is a key factor in the maximum wire delay and
interconnect power consumption.}
Stochastic wire length distributions have been shown to be effective tools for the rapid prediction
of interconnect properties in conventional 2DICs
\cite{davis_stochastic_1998,davis_stochastic_1998-1,venkatesan_performance_1999,sekar_intsim_2007}.
Efforts have been made to extend 2D wire length distributions to 3DICs
\cite{rahman_wire-length_1999-1,joyner_three-dimensional_2000, joyner_impact_2001, kim_through-silicon-via_2009-1},
...
two gates separated by length $l$ will be connected. 
\changed{$M(l)$ can be calculated relatively straightforwardly by dividing the die area into a grid of potential
logic gate locations, and simply counting all possible combinations of logic gates separated by a distance $l$.
The calculation of the connection probability, $P_c(l)$ relies upon Rent's Rule, the observation that
the number of terminals leaving a logic block, $T$, and the number of logic gates contained within that block, $N$, 
are related by the power law expression $T = kN^p$,
where $k$ is known as the Rent constant, and is roughly related to the average fanout of the logic, 
and $p$ is known as the Rent exponent, and is related to the degree of interconnectedness of the logic
\cite{landman_pinblock_1971,christie_interpretation_2000}. Using Rent's rule to calculate $P$ enables estimation
of the on-chip interconnect network parameters with a minimum of a priori knowledge about the design specifics.}
The general method of determining $M$ and $P$
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=2.0in]{Figures/thermal_config_no_bc_3.png}
	\caption{
...
interposer). Each die is separated into three regions: 1) the bulk silicon, 2) the BEOL, which is an effective material composed of the
...
of $5$ W/m$^2$K, whereas the top and bottom surfaces are given higher values to reflect the particular cooling method under consideration.}
...
Overall, this level of accuracy is sufficient for the investigation of power and performance trends in 2D and 3DICs.}
...
leading to increased stress on the cooling system, and eventually to unreasonable operating temperatures.
Since most conventional 2D ICs are already operating at their thermal limits, a system transformed from 2D to 3D must necessarily either
...
\changed{in both 2D and 3D configurations. Since 3DICs may require aggressive cooling strategies, the CPU core was simulated
with both air-cooled and microfluidic heat sinks. In all cases the heat sinks are located on the back side of the top die
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=3.5in]{Figures/sb3d_freq_vs_tiers.png}
	\caption{
		Maximum frequency achievable by 2D and 3D 32nm Sandy Bridge CPU cores limited to 90$^\circ$C under air cooling (blue)
		and microfluidic cooling (yellow).
		}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=3.5in]{Figures/sb3d_air_vs_water_epc.png}
	\caption{
		Core-level energy consumption per clock cycle for 2D and 3D Sandy Bridge CPU cores limited to 90$^\circ$C with air-cooled and microfluidic heat sinks.
		The power consumed by the cooling solution is not included in this comparison.
		}
	\label{f-3d-epc-air-vs-water}
\end{figure}

While the water-cooled designs can be run faster than air-cooled designs, they typically \changed{incur a slight reduction
in energy efficiency,}
as shown in \cref{f-3d-epc-air-vs-water}.
\changed{In all cases the 3D cores exhibit energy efficiency savings and performance increases over the 2D implementations.
The cores using microfluidic coolers exhibit slightly lower energy efficiencies than their air-cooled counterparts
since they were designed to take advantage of their greater thermal headroom by operating at higher frequencies.
which ultimately requires a greater overall fraction of die power to be used for on-chip communication, as previously shown in
\cref{f-sb-comm-power-fraction}}

%\begin{figure}[!htbp]
%	\centering
%	\includegraphics[width=3.5in]{Figures/sb3d_thermal_response.png}
%	\caption{
%		Thermal response of 2D and 3D 32nm Sandy Bridge CPU cores with air-cooled heat sinks.
%		}
%	\label{f-sb3d-thermal-response}
%end{figure}
