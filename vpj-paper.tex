...
leakage parameters, wiring material resistivity and size effect parameters, and expected die area and thickness.
...
The user must also specify the Rent constant, $k$, and Rent exponent, $p$,
which are described in greater detail in \cref{s-interconnect-modeling}.}

\begin{figure}[tb]
	\centering
	\includegraphics[width=3.0in]{Figures/vp-flowchart-3.png}
	\caption{Block diagram of the simulation platform execution flow.}
	\label{f-vp-flowchart}
\end{figure}

\changed{The Rent parameters are used to determine the number of TSVs required for routing signals within a 3DIC. 
...
the wire pitch on each tier, and the size and number of repeaters which must be inserted into long wires to meet
the target clock frequency.}

\changed{
For the purposes of on-chip interconnect modeling, the logic block under consideration is treated as a homogeneous design.
...
loss of insight into the fine-grained details of the design \cite{davis_stochastic_1998,joyner_impact_2001,sekar_intsim_2007}.
...
a heterogeneous system. When modeling heterogeneous designs, the interconnections
...
it is used in conjunction with the TSV resistance and inductance
...
if the demand for TSVs outstrips available area, the TSV diameter is reduced and the interconnect and power modules are rerun with
...
Once the design under consideration satisfies both the TSV area
...
Stochastic wire length distributions determine the number of wires of a particular length as $I_{idf}(l) = M(l)P_c(l)$,
...
\changed{$M(l)$ can be calculated by dividing the die area into a grid of potential
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=3.0in]{Figures/thermal_fd_setup.png}
	\caption{
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=2.0in]{Figures/thermal_config_no_bc_4.png}
	\caption{
...
of $5$ mW/m$^2$K, whereas the top and bottom surfaces are given higher values to reflect the particular cooling method under consideration.}
...
Core i7 680 is the only design under consideration which was implemented as a multi-chip module (MCM), with a 32nm CPU die 
...
in its ability to consider many different effects very rapidly (a single design scenario can be evaluated in several
...
\changed{In all cases, the microfluidically-cooled cores run faster than their air-cooled counterparts. In \cref{f-sb3d-epc-vs-tiers} the energy efficiency of the 32nm
Sandy Bridge CPU core is shown. Both the air-cooled and microfluidically-cooled 3D designs are more energy efficient than the 2D designs.
These results suggest that an aggressively-cooled high performance 3D logic core could simultaneously achieve higher performance and energy
efficiency than a conventional air-cooled 2DIC.}
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=3.5in]{Figures/sb3d_air_vs_water_epc_2.png}
	\caption{
		Energy consumed per clock cycle for 2D and 3D 32nm Sandy Bridge CPU cores under air cooling (blue) and microfluidic cooling (red).
		}
	\label{f-sb3d-epc-vs-tiers}
\end{figure}
